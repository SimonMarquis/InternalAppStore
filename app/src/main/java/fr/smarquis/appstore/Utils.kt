...
class Utils {
    ...
    companion object {
        ...
        fun isAtLeast(versionCode: Int): Boolean = SDK_INT >= versionCode

        fun isApplicationInstalled(context: Context, packageName: String): Boolean = try {
        ...
        fun applicationVersionName(context: Context, packageName: String): String? = try {
            context.packageManager.getPackageInfo(packageName, 0).versionName
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }

        fun applicationVersionCode(context: Context, packageName: String): Long = try {
            PackageInfoCompat.getLongVersionCode(context.packageManager.getPackageInfo(packageName, 0))
        } catch (e: PackageManager.NameNotFoundException) {
            0
        }

        fun applicationLastUpdateTime(context: Context, packageName: String): Long? = try {
            context.packageManager.getPackageInfo(packageName, 0).lastUpdateTime
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }

        fun getLaunchIntent(context: Context, packageName: String): Intent? = context.packageManager.getLaunchIntentForPackage(packageName)

        fun getDetailsIntent(packageName: String): Intent = Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.fromParts("package", packageName, null))

        fun getDeleteIntent(packageName: String): Intent = Intent(Intent.ACTION_DELETE, Uri.fromParts("package", packageName, null))

        fun getMarketIntent(packageName: String): Intent = Intent(Intent.ACTION_VIEW, Uri.Builder().scheme("market").authority("details").appendQueryParameter("id", packageName).build())

        fun notificationSettingsIntent(context: Context, channelId: String? = null, openChannel: Boolean = true): Intent = when {
            SDK_INT >= O -> {
        ...
        fun parseHtml(description: String?): Spanned? = if (description.isNullOrBlank()) {
            null
        } else {
            description.replace("\n", "<br>").let {
                if (isAtLeast(N)) {
        ...
        fun relativeTimeSpan(time: Long?, now: Long = System.currentTimeMillis(), minResolution: Long = SECOND_IN_MILLIS, flags: Int = FORMAT_ABBREV_RELATIVE): CharSequence? =
            DateUtils.getRelativeTimeSpanString(time ?: now, now, minResolution, flags)
    }
}
