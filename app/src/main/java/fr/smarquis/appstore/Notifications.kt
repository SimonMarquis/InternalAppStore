    ...
    companion object {
        ...
        fun createOrUpdateNewApplicationsNotificationChannel(context: Context) {
            ...
            notificationManager(context).createNotificationChannel(
                NotificationChannel(
                    context.getString(R.string.notification_channel_new_applications_id),
                    context.getString(R.string.notification_channel_new_applications_name),
        ...
        fun createOrUpdateNewVersionsNotificationChannel(context: Context, application: Application) {
            ...
            if (nm.getNotificationChannel(channelId) == null) {
                ...
                val shouldMuteNotificationChannel =
                    nm.getNotificationChannel(context.getString(R.string.notification_channel_new_applications_id))?.importance == NotificationManagerCompat.IMPORTANCE_NONE
                nm.createNotificationChannel(
                    NotificationChannel(
                        channelId,
                        application.name,
                        if (application.isMyself(context)) NotificationManager.IMPORTANCE_HIGH else if (shouldMuteNotificationChannel) NotificationManager.IMPORTANCE_NONE else NotificationManager.IMPORTANCE_LOW,
                    ),
                )
            } else {
                nm.createNotificationChannel(
                    NotificationChannel(
                        channelId,
                        application.name,
                        if (application.isMyself(context)) NotificationManager.IMPORTANCE_HIGH else NotificationManager.IMPORTANCE_LOW,
                    ),
                )
            }
        }
        ...
        private fun notifyWithImage(context: Context, tag: String, id: Int, notificationBuilder: NotificationCompat.Builder, image: Any?) {
            ...
            Handler(Looper.getMainLooper()).post {
                Glide.with(context).asBitmap().apply(requestOptions).load(image)
                    .into(
                        object : CustomTarget<Bitmap>(width, height) {
